...
class FieldTupleSpec extends Spec with MustMatchers {
  ...
  describe("valuesTuple") {
    it("must extract the field values") {
      ...
      fieldTuple.valuesTuple(PortableField.UseDefaults) match {
      ...
      val (integer, string, double) = fieldTuple.valuesTuple(PortableField.UseDefaults)
    ...
    it("must work for tuple size of 2") {
      ...
      tuple.valuesTuple(PortableField.UseDefaults) match {
